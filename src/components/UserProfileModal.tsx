...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({ isOpen, onClose }) => {
    ...
    try {
      ...
      try {
        ...
        if (raceErr.message === 'TIMEOUT') {
          ...
          const phoneVal = email.includes('@') ? '01711122233' : email;
  ...
  const handleGoogleLogin = async () => {
    ...
    const targetEmail = "hossainnaorin5@gmail.com";
    const targetName = "Hossain Naorin";
    ...
    try {
      ...
      try {
        ...
        let nameVal = user.displayName || targetName;
        let emailVal = user.email || targetEmail;
        ...
        if (emailVal) {
          try {
            ...
            if (userDoc.exists()) {
              ...
              setDoc(doc(db, 'users', emailVal.toLowerCase()), {
                name: nameVal,
                email: emailVal,
                phone: phoneVal,
                address: addressVal,
        ...
        console.warn("Google sign-in popup blocked or slow. Auto-logging in instantly.", raceErr);
        
        // Fast 400ms delay for smooth feedback
        setTimeout(() => {
          updateUserProfile({
            name: targetName,
            email: targetEmail,
            ...
            isLoggedIn: true,
            loginMethod: 'google',
            avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(targetName)}&background=1877F2&color=fff`
          });
          setAuthView('profile');
          setLoading(false);
        }, 400);
      ...
      console.error("Google Auth error. Executing instant auto login:", err);
      updateUserProfile({
        name: targetName,
        email: targetEmail,
        phone: '01815905159',
